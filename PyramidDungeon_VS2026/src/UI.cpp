...
std::string CategoryLabel(const std::string& category) {
    ...
    if (category == "material") return "Matériaux";
...
void UI::DrawHud(const Player& player, const Inventory& inventory, int floor, const Room* room, const Boss& boss, const std::string& message) {
    ...
    if (player.maxResource > 0.0f) {
        ...
        std::string res = player.classType == ClassType::Necromancer ? "Âme" : (player.classType == ClassType::Monk ? "Ki" : "Mana");
    ...
    DrawText(("Étage : " + std::to_string(floor)).c_str(), x + hudW / 2, y + 122, 17, RAYWHITE);
...
void UI::DrawHelp() {
    const UiMetrics m = Metrics();
    const char* text = "Options : touches/FOV | Souris : regard/menu | Espace : saut | Shift : sneak | Ctrl : dash | Clic gauche : attaque/valider | Échap : pause";
...
void UI::DrawClassSelection(int selected) {
    std::vector<std::string> lines = {
        (selected == 0 ? "> " : "  ") + std::string("Mage élémentaire - projectiles, explosion, mana"),
        (selected == 1 ? "> " : "  ") + std::string("Nécromancien - trait sombre, invocation, énergie d'âme"),
        (selected == 2 ? "> " : "  ") + std::string("Combattant - mêlée, charge, grosse défense"),
...
void UI::DrawShopMenu(const std::string& title, const Shop& shop, const Inventory& inventory, int selected) {
    ...
    lines.push_back("Entrée : acheter | Échap : quitter");
    lines.push_back("");
...
void UI::DrawVendorMenu(const Vendor& vendor, const Inventory& inventory, int selected) {
    ...
    lines.push_back("Entrée : vendre 1 | V : vendre communs auto | Échap : quitter");
    lines.push_back("");
...
void UI::DrawCraftingMenu(const CraftingStation& station, const Inventory& inventory, int selected) {
    std::vector<std::string> lines;
    lines.push_back("Entrée : transformer | Échap : quitter");
    lines.push_back("");
    const auto& recipes = station.Recipes();
    for (size_t i = 0; i < recipes.size(); ++i) {
        const auto& r = recipes[i];
        std::string line = (static_cast<int>(i) == selected ? "> " : "  ") + r.name + " | Coût : " + std::to_string(r.goldCost) + " or";
...
void UI::DrawUpgradeMenu(const UpgradeStation& station, const UpgradeData& upgrades, const Inventory& inventory, int selected) {
    std::vector<std::string> lines;
    lines.push_back("Or : " + std::to_string(inventory.gold));
    lines.push_back("Entrée : améliorer | Échap : quitter");
    lines.push_back("");
    ...
    DrawCenteredPanel("Autel d'amélioration", lines);
}
...
void UI::DrawInventoryMenu(const Inventory& inventory, int selected) {
    ...
    DrawTextFit("Entrée : équiper l'arme | A/R utilisables en jeu | Tab/Échap : fermer", panelX + 24, panelY + 94, 16, LIGHTGRAY, gridW);
    ...
    if (!items.empty()) {
        ...
        drawDetail("Catégorie : " + CategoryLabel(def ? def->category : "other"), 17, WHITE);
        drawDetail("Quantité : " + std::to_string(current.second), 17, LIGHTGRAY);
        drawDetail("Valeur : " + std::to_string(def ? def->sellPrice : 0) + " or/u", 17, GoldLine());
        drawDetail(def && def->rare ? "Rareté : rare" : "Rareté : commune", 17, def && def->rare ? GoldLine() : LIGHTGRAY);
        if (def && def->category == "weapon") {
            drawDetail("Dégâts +" + std::to_string(static_cast<int>(def->damageBonus)), 17, WHITE);
            drawDetail("Portée +" + std::to_string(static_cast<int>(def->rangeBonus * 10.0f) / 10.0f), 17, WHITE);
            drawDetail("Vitesse " + std::to_string(static_cast<int>(def->attackSpeedBonus * 100.0f)) + "%", 17, WHITE);
            drawDetail(inventory.equippedWeapon == current.first ? "Action : déjà équipée" : "Action : Entrée pour équiper", 17, Turquoise());
        } else {
...
void UI::DrawChestMenu(const Inventory& inventory, int selected, bool chestSide) {
    std::vector<std::string> lines;
    lines.push_back(chestSide ? "Coffre sélectionné - Entrée : retirer 1 | C : changer de côté" : "Inventaire sélectionné - Entrée : déposer 1 | C : changer de côté");
    lines.push_back("Échap : fermer");
    lines.push_back("");
    ...
    DrawTextFit("Changer de côté", static_cast<int>(button.x) + 12, static_cast<int>(button.y) + 5, 15, WHITE, 150);
}
